...
function placeXorO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //this con/ checks whos turn it is
        if (activePlayer === 'X') {
            //if activePlayer is equal to 'X', the x.png is placed in HTML
            select.style.backgroundImage = 'url("images/x.png")';
            //active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
            //if activeplayer is equal to 'O', the o.png is placed in HMTL
        ...
        //this con checks to see if it is the computers turn
        if (activePlayer === 'O') {
    ...
    function computersTurn() {
        ...
        let pickASquare;
        //this con allows our while loop to keep trying if a square is selected already
        while (!success) {
...
//drawline() function is called to drawca line on the screen if the condition is met
...
function resetGame() {
    ...
    //this resets our array so it is emtpy and we can start over
    selectedSquares = [];
}